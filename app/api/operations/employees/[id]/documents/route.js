...
export async function POST(request, { params }) {
  ...
  const storageKey = `${id}/${crypto.randomUUID()}-${safeName(file.name)}`; const fullPath = path.join(ROOT, storageKey); await fs.mkdir(path.dirname(fullPath), { recursive: true }); await fs.writeFile(fullPath, Buffer.from(await file.arrayBuffer()));
