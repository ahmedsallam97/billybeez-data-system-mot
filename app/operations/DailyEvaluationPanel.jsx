...
function draftFor(evaluation, criteria) {
  ...
  return {
    ...
    penaltyNote: evaluation.penaltyNote || "",
    guidanceNote: evaluation.guidanceNote || "",
  };
}
...
function EvaluationEditor({ evaluation, criteria, closed, isArabic, penaltyDeduction, guidanceDeduction, onClose, onSave }) {
  ...
  const [penaltyNote, setPenaltyNote] = useState(evaluation.penaltyNote || "");
  const [guidanceNote, setGuidanceNote] = useState(evaluation.guidanceNote || "");
  const [reason, setReason] = useState("");
  const score = Math.max(0, evaluation.maxScore - Object.values(values).reduce((sum, item) => sum + (Number(item.deduction) || 0), 0) - (penaltyNote.trim() ? penaltyDeduction : 0) - (guidanceNote.trim() ? guidanceDeduction : 0));
...
export default function DailyEvaluationPanel({ date }) {
  ...
  const guidanceDeduction = Math.max(0, Number(data?.rules?.guidanceDeduction ?? 15));
  ...
  return <section className="panel"><div className="operations-profile-head"><div><h2>{isArabic ? "التقييم اليومي والمظهر" : "Daily Evaluation & Grooming"}</h2><span>{data?.criteriaVersion?.label || ""}</span></div>{data?.day && <em>{data.day.status}</em>}</div>
    ...
    {!data?.day ? <div className="muted">{data?.team.length ? (isArabic ? "جارٍ تجهيز تقييمات الفريق تلقائيًا بدرجة 10/10..." : "Preparing the team evaluations automatically at 10/10...") : (isArabic ? "لا يوجد فريق عامل في الجدول المنشور" : "No working team in the published schedule")}</div> : <>
      <div className="record-table-scroll evaluation-table-wrap"><table className="record-line-table evaluation-table evaluation-ready-table"><thead><tr><th>{isArabic ? "الموظف" : "Employee"}</th>{criteria.map((criterion) => <th key={criterion.id}>{isArabic ? (criterion.labelAr || ARABIC_CRITERIA[criterion.code] || criterion.label) : criterion.label}<small>{criterion.maxScore}</small></th>)}<th>{isArabic ? "الإجمالي" : "Total"}</th><th>{isArabic ? "الحفظ" : "Save"}</th><th>{isArabic ? "التفاصيل" : "Details"}</th></tr></thead><tbody>{data.day.evaluations.map((item) => {
        const draft = drafts[item.employeeId] || draftFor(item, criteria);
        const total = Math.max(0, criteria.reduce((sum, criterion) => sum + Number(draft.scores?.[criterion.id] ?? criterion.maxScore), 0) - (draft.penaltyNote?.trim() ? penaltyDeduction : 0) - (draft.guidanceNote?.trim() ? guidanceDeduction : 0));
        return <tr key={item.id}><td><b>{item.employee.name}</b><small>{item.status === "DEFAULT_FULL" ? (isArabic ? "جاهز بالقيمة الافتراضية" : "Ready at default") : item.status}</small><span className="evaluation-action-badges">{draft.penaltyNote?.trim() && <i className="penalty">{isArabic ? "جزاء" : "Penalty"} -{penaltyDeduction}</i>}{draft.guidanceNote?.trim() && <i className="guidance">{isArabic ? "توجيه" : "Guidance"} -{guidanceDeduction}</i>}</span></td>{criteria.map((criterion) => <td key={criterion.id}><input aria-label={`${item.employee.name} ${criterion.code}`} type="number" min="0" max={criterion.maxScore} step="0.5" disabled={data.day.status === "CLOSED"} value={draft.scores?.[criterion.id] ?? criterion.maxScore} onChange={(event) => updateScore(item.employeeId, criterion, event.target.value)} /></td>)}<td><b>{total}/{maximum}</b></td><td>{data.day.status !== "CLOSED" && <button onClick={() => action({ action: "save", evaluations: [evaluationPayload(item)] }, isArabic ? `تم حفظ تقييم ${item.employee.name}` : `${item.employee.name} saved`)}>{isArabic ? "حفظ" : "Save"}</button>}</td><td><button className="secondary" onClick={() => setSelected(item)}>{data.day.status === "CLOSED" ? (isArabic ? "عرض / تصحيح" : "View / correct") : (isArabic ? "جزاءات وتوجيه" : "Actions & notes")}</button></td></tr>;
      })}</tbody></table></div>
    ...
    {selected && <EvaluationEditor evaluation={selected} criteria={criteria} closed={data.day.status === "CLOSED"} isArabic={isArabic} penaltyDeduction={penaltyDeduction} guidanceDeduction={guidanceDeduction} onClose={() => setSelected(null)} onSave={action} />}
  </section>;
}
