...
export function normalizeDepartmentId(value) {
  ...
  return normalized === "RESTAURANT" ? "KITCHEN" : normalized;
}

function normalizeDepartmentKind(value) {
  return ["KITCHEN", "RESTAURANT"].includes(String(value || "").toUpperCase()) ? "KITCHEN" : "DATA";
}
...
export function isDataDepartment(department, departments) {
  ...
  return normalized.some((item) => item.id === departmentId && item.kind === "DATA" && item.active);
}

export function isRestaurantDepartment(department, departments) {
  const normalized = normalizeEmployeeDepartments(departments);
  const departmentId = normalizeDepartmentId(department);
  return normalized.some((item) => item.id === departmentId && item.kind === "KITCHEN" && item.active);
}
