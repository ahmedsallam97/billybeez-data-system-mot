...
const NAMED_COLORS = [
  ["Light Purple", "#a154e8"], ["Purple", "#7e3fb2"], ["Lavender", "#c7a4ef"],
  ["Red", "#e05252"], ["Pink", "#e77aa8"], ["Light Pink", "#f2b6cf"],
  ["Blue", "#3f7fd1"], ["Light Blue", "#72b7e5"], ["Sky Blue", "#82c9dc"],
  ["Green", "#4fa968"], ["Light Green", "#8dcea1"], ["Yellow", "#f2c94c"],
  ["Orange", "#e99a45"], ["Brown", "#8b5a3c"], ["Black", "#222222"],
  ["White", "#ffffff"], ["Gray", "#8b8b8b"],
];

function colorNameFor(value) {
  const color = String(value || "").trim().toLowerCase();
  if (!/^#[0-9a-f]{6}$/.test(color)) return String(value || "").trim() || "Color";
  const rgb = [1, 3, 5].map((index) => parseInt(color.slice(index, index + 2), 16));
  return NAMED_COLORS
    .map(([name, hex]) => {
      const target = [1, 3, 5].map((index) => parseInt(hex.slice(index, index + 2), 16));
      const distance = rgb.reduce((sum, component, index) => sum + (component - target[index]) ** 2, 0);
      return { name, distance };
    })
    .sort((left, right) => left.distance - right.distance)[0].name;
}

module.exports = { ALL_WEEKDAYS, normalizeWeekdays, offerAppliesOnDate, stockAvailable, stockIssueUpdate, stockCanDelete, selectBraceletStock, stockKey, colorNameFor };
