...
const DEFAULT_DAILY_OPERATIONS_TEMPLATE_CONFIG = {
  logoUrl: "/bb-logo-fast.png", branchName: "MOT Branch", title: "DAILY OPERATIONS", subtitle: "Great teams make great days.", footerMotto: "Great teams make great days.",
  primary: "#301848", accent: "#f8c800", text: "#20113d", amColor: "#cfe8ff", bwColor: "#fff2ac", pmColor: "#eadcff",
  cardOrder: ["trips", "birthdays", "offers", "bracelets"], visibleCards: { trips: true, birthdays: true, offers: true, bracelets: true },
  sectionOrder: ["roster", "leaves", "notes", "footer"], visibleSections: { roster: true, leaves: true, notes: true, footer: true, attendance: true, breaks: true, rotation: true },
  cardContent: { trips: "No trips added", birthdays: "No birthdays added", offers: "No offers added", bracelets: "Kids: Red\nToddlers: Light blue\nTrip: Green\nS.N: Purple\nVisitor: Brown" },
  ...
  currency: "EGP",
  weekdayLocale: "en-US",
  fillerRows: { AM: 5, BW: 4, PM: 5 }, rotationHours: ["10", "11", "12", "1", "2", "3", "4", "5"],
};
...
const DEFAULT_OPS_CASHIER_CONFIG = {
  primaryEmployeeIds: [],
  backupEmployeeIds: [],
  primaryCount: 3,
  backupCount: 4,
};
const DEFAULT_OPS_BRANCH_CONFIG = { branchCode: "MOT", branchName: "MOT Branch", timezone: "Africa/Cairo", locale: "en-GB" };
const DEFAULT_OPS_SCHEDULE_CODE_CONFIG = {
  AM: { label: "AM", color: "#f4dda0" }, BW: { label: "BW", color: "#b9ddf2" }, PM: { label: "PM", color: "#abc5e5" },
  Annual: { label: "Annual", color: "#b9dfc3" }, Rep: { label: "Rep", color: "#efd992" }, OFF: { label: "OFF", color: "#efc1ce" }, SL: { label: "SL", color: "#ebbac6" },
  Unpaid: { label: "Unpaid", color: "#f2c392" }, Mission: { label: "Mission", color: "#b9e3ec" }, Holiday: { label: "Holiday", color: "#d5afbf" },
  ANP: { label: "ANP", color: "#efb8c0" }, AWP: { label: "AWP", color: "#ce8593" }
};
const DEFAULT_OPS_ROTATION_RULES = { slotsPerShift: 8, preserveManualLocks: true, excludeCashiers: true, prioritizeMandatoryPositions: true, mandatoryPositionCodes: ["DATA"], highlightedPositionCodes: ["DROP", "TOWER", "DATA"], avoidConsecutivePosition: true, optionalOnlyWhenFullyStaffed: true, earlyTripRule: { enabled: true, triggerTime: "09:00", shiftCode: "AM", startTime: "09:00", endTime: "17:00" } };
const DEFAULT_OPS_ATTENDANCE_RULES = { lateGraceMinutes: 10, earlyLeaveGraceMinutes: 10, allowBulkExpectedTimes: true, requireReasonAfterClose: true };
const DEFAULT_OPS_EVALUATION_RULES = { defaultScoreMode: "FULL_UNREVIEWED", requireReasonForDeduction: true, requireReviewBeforeClose: true };
const DEFAULT_OPS_LEAVE_RULES = { overtimeHoursPerDay: 8, holidayWorkCreditDays: 2, preventNegativeBalance: true, requireCoverageCheck: true };
const DEFAULT_OPS_STOCK_RULES = { lowStockThreshold: 10 };
const DEFAULT_OPS_PLANNING_CATALOGS = {
  meals: ["Chicken Nuggets", "Beef Burger", "Chicken Burger"],
  partyRooms: [{ name: "Party Room", capacity: 0, active: true }],
  stockItems: ["Socks M Yellow", "Socks M Pink", "Socks L Yellow", "Socks L Pink", "Bracelet Satan", "Bracelet Paper", "Bracelet Plastic", "Cash Roll", "Visa Roll"],
  stockCatalog: {
    socksSizes: ["M", "L"],
    socksColors: [{ label: "Yellow", value: "#f2c94c" }, { label: "Pink", value: "#e77aa8" }],
    braceletMaterials: [{ code: "SATAN", label: "Satan" }, { code: "PAPER", label: "Paper" }, { code: "PLASTIC", label: "Plastic" }],
    braceletUsages: [{ code: "KID", label: "Kid" }, { code: "TODDLER", label: "Toddler" }, { code: "S_N", label: "S.N" }, { code: "VISITOR", label: "Visitor" }, { code: "TRIP", label: "Trip", system: true }, { code: "BIRTHDAY", label: "Birthday", system: true }],
    rollStyles: [{ code: "PRINTED", label: "Printed" }, { code: "PLAIN", label: "Plain" }],
  },
};

const DEFAULT_RECOGNITION_ARTWORK_CONFIG = {
  theme: "CLASSIC",
  global: {
  ...
  employeeNameOverrides: {},
  monthly: {
    template: "T01",
    brandTagline: "PLAY · LEARN · GROW",
    titleLine1: "Employee",
    titleLine2: "of The Month",
    congratulations: "Congratulations, {firstName}!",
    message: "We are proud to have you on our team.",
    leftMessage: "SAME\nTEAM\nBIGGER\nSMILES",
    rightMessage: "GREAT\nPEOPLE\nBRIGHTER\nTOMORROWS",
    nameFontFamily: '"Avenir Next", "Segoe UI", Arial, sans-serif',
    nameFontSize: 34,
    nameFontWeight: 950,
    nameLetterSpacing: 0,
    nameColor: "#ffffff",
    nameBackground: "linear-gradient(100deg,#7a36bd,#2c076b)",
    nameMaxLines: 2,
    photoFit: "cover",
  ...
  hall: {
    slogan: "GREAT\nPEOPLE\nBRIGHTER\nTOMORROWS",
    title: "HALL OF FAME",
    subtitle: "OUR EMPLOYEES OF THE MONTH",
    intro: "A YEAR OF DEDICATION, PASSION AND IMPACT",
    messageTitle: "To Our Amazing Team,",
    ...
    messageHighlight: "Thank you for being the heart of our success!",
    messageFooter: "Here’s to an even more exciting year ahead — more growth, more achievements, and many more smiles together!",
    signatureLabel: "With Appreciation,",
    signature: "Billy Beez Management",
    awaitingWinner: "Awaiting finalized winner",
    nameFontFamily: '"Avenir Next", "Segoe UI", Arial, sans-serif',
    nameFontSize: 11,
    nameFontWeight: 900,
    nameLetterSpacing: 0,
    ...
    photoFit: "cover",
    photoPosition: "center",
    oddMonthColor: "#54209a",
    evenMonthColor: "#f8c800",
    branchSuffix: "Branch",
    monthLabels: ["JANUARY", "FEBRUARY", "MARCH", "APRIL", "MAY", "JUNE", "JULY", "AUGUST", "SEPTEMBER", "OCTOBER", "NOVEMBER", "DECEMBER"],
    values: [
      { icon: "♢", line1: "Safety", line2: "Always" },
      { icon: "♚", line1: "Stronger", line2: "Together" },
      { icon: "▥", line1: "Bigger", line2: "Possibilities" },
      { icon: "♥", line1: "Happier", line2: "Guests" },
      { icon: "★", line1: "Brighter", line2: "Future" },
    ],
  },
};
...
const DEFAULT_SETTINGS = Object.freeze({
  ...
  DAILY_OPERATIONS_TEMPLATE_CONFIG: {
    value: JSON.stringify(DEFAULT_DAILY_OPERATIONS_TEMPLATE_CONFIG),
    description: "Live editable Daily Operations roster template",
  },
  OPS_MOTIVATION_PHRASES: { value: JSON.stringify(DEFAULT_OPS_MOTIVATION_PHRASES), description: "Daily Operations rotating motivational phrases" },
  OPS_CASHIER_CONFIG: { value: JSON.stringify(DEFAULT_OPS_CASHIER_CONFIG), description: "Dedicated and backup cashier roster configuration" },
  OPS_BRANCH_CONFIG: { value: JSON.stringify(DEFAULT_OPS_BRANCH_CONFIG), description: "Operations branch and timezone configuration" },
  OPS_SCHEDULE_CODE_CONFIG: { value: JSON.stringify(DEFAULT_OPS_SCHEDULE_CODE_CONFIG), description: "Operational schedule code labels and colors" },
  OPS_ROTATION_RULES: { value: JSON.stringify(DEFAULT_OPS_ROTATION_RULES), description: "Automatic rotation generation rules" },
  OPS_ATTENDANCE_RULES: { value: JSON.stringify(DEFAULT_OPS_ATTENDANCE_RULES), description: "Attendance thresholds and workflow rules" },
  OPS_EVALUATION_RULES: { value: JSON.stringify(DEFAULT_OPS_EVALUATION_RULES), description: "Daily evaluation preparation and close rules" },
  OPS_LEAVE_RULES: { value: JSON.stringify(DEFAULT_OPS_LEAVE_RULES), description: "Leave, holiday and overtime operational rules" },
  OPS_STOCK_RULES: { value: JSON.stringify(DEFAULT_OPS_STOCK_RULES), description: "Operations inventory alert thresholds" },
  OPS_PLANNING_CATALOGS: { value: JSON.stringify(DEFAULT_OPS_PLANNING_CATALOGS), description: "Meals, party rooms and operations stock catalogue" },
  RECOGNITION_ARTWORK_CONFIG: { value: JSON.stringify(DEFAULT_RECOGNITION_ARTWORK_CONFIG), description: "Employee of the Month and Hall of Fame artwork themes, text, typography and colors" },
});
...
async function ensureDefaultSettings(client = prisma) {
  const writeDefaults = async () => {
    const existingRows = await client.systemSetting.findMany({
      select: { key: true },
    });
    ...
    for (const [key, setting] of missingSettings) {
      createdSettings.push(await client.systemSetting.upsert({
        where: { key },
        create: {
          key,
          value: setting.value,
...
module.exports = {
  ...
  DEFAULT_RECOGNITION_ARTWORK_CONFIG,
  clearSettingsCache,
