...
const DEFAULT_ROTATION_RULES = {
  ...
  highlightedPositionCodes: ["DROP", "TOWER", "DATA"],
  avoidConsecutivePosition: true,
  optionalOnlyWhenFullyStaffed: true,
  distributeAvailableEmployees: true,
};
...
function normalizeRotationRules(value = {}) {
  return {
    ...
    ...(value || {}),
    mandatoryPositionCodes: Array.isArray(value?.mandatoryPositionCodes) ? value.mandatoryPositionCodes : DEFAULT_ROTATION_RULES.mandatoryPositionCodes,
    highlightedPositionCodes: Array.isArray(value?.highlightedPositionCodes) ? value.highlightedPositionCodes : DEFAULT_ROTATION_RULES.highlightedPositionCodes,
  };
}
...
function generateRotation({
  ...
  const highlightedPositions = new Set(normalizedRules.highlightedPositionCodes);
  ...
  for (const slot of slots) {
    ...
    const requirements = activeRequirements(positions, slot.shiftCode, workDate)
      ...
      if (normalizedRules.prioritizeMandatoryPositions && leftMandatory !== rightMandatory) return rightMandatory - leftMandatory;
    ...
    for (const { position, requirement } of requirements) {
      ...
      const pool =
        position.code === "CASHIER"
          ...
          : working.filter(
              (member) =>
                member.shiftCode === slot.shiftCode &&
                (!normalizedRules.excludeCashiers || (!member.frontAssignment && member.employee?.department !== "CASHIER")) &&
      ...
      for (let index = 0; index < count; index += 1) {
        ...
        assignments.push({
          ...
          highlight: highlightedPositions.has(position.code),
        });
        used.set(
    ...
    // Staffing requirements guarantee mandatory coverage, but they should not
    // leave the rest of a fully staffed team without a visible distribution.
    // Fill every remaining eligible employee with a distinct operational
    // position for the hour and rotate the choices deterministically.
    if (normalizedRules.distributeAvailableEmployees !== false) {
      const availableMembers = working
        .filter((member) =>
          member.shiftCode === slot.shiftCode &&
          (!normalizedRules.excludeCashiers || (!member.frontAssignment && member.employee?.department !== "CASHIER")) &&
          !member.employee?.operationsTeamLeader &&
        ...
        .sort((left, right) => (used.get(left.employeeId) || 0) - (used.get(right.employeeId) || 0) || seededRank(`${workDate}:${slot.startTime}:${left.employeeId}`) - seededRank(`${workDate}:${slot.startTime}:${right.employeeId}`));

      const occupiedPositions = new Set(assignments
        .filter((assignment) => overlaps(assignment.startTime, assignment.endTime, slot.startTime, slot.endTime))
        .map((assignment) => assignment.operationalPositionId));

      for (const member of availableMembers) {
        const candidate = positions
          .filter((position) =>
            position.code !== "CASHIER" &&
            !occupiedPositions.has(position.id) &&
            canAssign({ qualifications, employeeId: member.employeeId, position }),
          )
          .sort((left, right) => {
            if (normalizedRules.avoidConsecutivePosition) {
              const leftRepeated = assignments.some((item) => item.employeeId === member.employeeId && item.operationalPositionId === left.id && item.endTime === slot.startTime);
              const rightRepeated = assignments.some((item) => item.employeeId === member.employeeId && item.operationalPositionId === right.id && item.endTime === slot.startTime);
              if (leftRepeated !== rightRepeated) return leftRepeated ? 1 : -1;
            }
            const leftUsage = assignments.filter((item) => item.employeeId === member.employeeId && item.operationalPositionId === left.id).length;
            const rightUsage = assignments.filter((item) => item.employeeId === member.employeeId && item.operationalPositionId === right.id).length;
            if (leftUsage !== rightUsage) return leftUsage - rightUsage;
            return seededRank(`${workDate}:${slot.startTime}:${member.employeeId}:${left.code}`) - seededRank(`${workDate}:${slot.startTime}:${member.employeeId}:${right.code}`);
          })[0];
        if (!candidate) continue;
        assignments.push({
          employeeId: member.employeeId,
          operationalPositionId: candidate.id,
          positionCode: candidate.code,
          shiftCode: slot.shiftCode,
          startTime: slot.startTime,
          endTime: slot.endTime,
          manualLock: false,
          highlight: highlightedPositions.has(candidate.code),
        });
        occupiedPositions.add(candidate.id);
        used.set(member.employeeId, (used.get(member.employeeId) || 0) + 1);
      }
    }
  }
...
function coverageFor({
  ...
  const highlightedPositions = new Set(normalizeRotationRules(rules).highlightedPositionCodes);
