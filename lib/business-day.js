...
const { upsertDailyClosingSnapshot } = require("./daily-snapshot");
const { ensureDefaultSettings, getNumberSetting } = require("./settings");

const TIME_ZONE = "Africa/Cairo";
const OPEN_HOUR = 7;
const CLOSE_HOUR = 1;
const BUSINESS_STATE_CACHE_TTL_MS = 10_000;
let businessStateCache = null;
let businessStateCacheExpiresAt = 0;

function clearBusinessDayStateCache() {
  businessStateCache = null;
  businessStateCacheExpiresAt = 0;
}

function cacheBusinessDayState(state) {
  businessStateCache = { ...state };
  businessStateCacheExpiresAt = Date.now() + BUSINESS_STATE_CACHE_TTL_MS;
  return { ...businessStateCache };
}
...
function getBusinessState(now = new Date(), settings = {}) {
  const parts = cairoParts(now);
  const today = dateString(parts);
  const openHour = Number(settings.openHour ?? OPEN_HOUR);
  const closeHour = Number(settings.closeHour ?? CLOSE_HOUR);

  if (parts.hour < closeHour) {
    return {
      isOpen: true,
    ...
    for (const order of orders) {
      ...
      await tx.orderHistory.upsert({
        ...
        create: {
          ...
          customerLeftAt: order.customerLeftAt || null,
...
async function ensureBusinessDayState(now = new Date()) {
  const useCache = arguments.length === 0;

  if (useCache && businessStateCache && businessStateCacheExpiresAt > Date.now()) {
    return { ...businessStateCache };
  }

  await ensureDefaultSettings();
  const [openHour, closeHour] = await Promise.all([
    getNumberSetting("BUSINESS_OPEN_HOUR", OPEN_HOUR),
    getNumberSetting("BUSINESS_CLOSE_HOUR", CLOSE_HOUR),
  ]);
  const today = dateString(cairoParts(now));
  let state = getBusinessState(now, { openHour, closeHour });
  ...
  if (state.businessDate) {
    ...
    if (currentDay?.closedAt) {
      ...
      return useCache ? cacheBusinessDayState(closedState) : closedState;
    }
  }
  ...
  return useCache ? cacheBusinessDayState(finalState) : finalState;
}
...
async function openBusinessDay(now = new Date()) {
  ...
  const baseState = getBusinessState(now, { openHour, closeHour });
...
async function closeActiveBusinessDay(now = new Date(), actor = null) {
  ...
  await upsertDailyClosingSnapshot(prisma, businessDate, orders);
