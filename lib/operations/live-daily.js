...
function generateRotation({
  ...
  for (const slot of slots) {
    const mandatoryCodes = new Set(rules.mandatoryPositionCodes || []);
    const requirements = activeRequirements(positions, slot.shiftCode, workDate).sort((left, right) => {
    ...
    for (const { position, requirement } of requirements) {
      ...
      const eligible = pool.filter(
        (member) =>
          ...
          !assignments.some(
            (assignment) =>
              assignment.employeeId === member.employeeId &&
              overlaps(
      ...
      ({ position, requirement }) => {
        ...
        return {
          ...
          shortage: Math.max(
            ...
            (position.code === "CASHIER" && shiftCode === "BW"
              ? 0
              : requirement.minEmployees) - assigned.length,
          ),
          highlight: STRONG_HIGHLIGHT_POSITIONS.has(position.code),
        };
      },
...
function buildReadiness({
  ...
  const remainingBracelets = wristbands.reduce(
    (sum, item) =>
      sum + Math.max(0, item.availableStock - item.allocated - item.issued),
    0,
  );
  checks.push({
    code: "TRIP_STAFFING",
