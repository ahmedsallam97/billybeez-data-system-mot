...
`D:\Projects\billybeez-system-backups\BillyBeez-MOT-restore-2026-09-24T08-52-16Z.zip`
...
```powershell
$Archive = 'D:\Projects\billybeez-system-backups\BillyBeez-MOT-restore-2026-09-24T08-52-16Z.zip'
$Extracted = 'D:\SecureRestore\BillyBeez-MOT-restore-2026-09-24T08-52-16Z'
...
```powershell
$Package = Join-Path $Extracted 'BillyBeez-MOT-restore-2026-09-24T08-52-16Z'
...
The package contains 12 protected employee files in 11 employee folders. Copy the whole tree; do not rename folders or files because database metadata points to their storage keys.
...
```powershell
npm run db:verify-backup -- .\backups\manual-2026-09-24T08-52-16-668Z.db
```
...
At package creation, employee storage contained 12 protected files across 11 employee folders. Confirm authenticated document/photo routes can open representative files after restoration.
...
Stop the server. Preserve the failed restored database for diagnosis, then replace `prisma\dev.db` with `backups\manual-2026-09-24T08-52-16-668Z.db`. Repeat database verification before restarting.
