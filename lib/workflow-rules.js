...
async function loadWorkflowRules() {
  return {
    ...
    allowPaymentBeforeDelivery: await getBooleanSetting("WORKFLOW_ALLOW_PAYMENT_BEFORE_DELIVERY", false),
    requireGeideaBeforeArchive: await getBooleanSetting("WORKFLOW_REQUIRE_GEIDEA_BEFORE_ARCHIVE", true),
    requirePaymentBeforeArchive: await getBooleanSetting("WORKFLOW_REQUIRE_PAYMENT_BEFORE_ARCHIVE", true),
...
function validateArchiveAllowed(order, rules) {
  if (rules.requirePaymentBeforeArchive && order.paymentStatus !== "PAID") {
    return workflowError("Order must be paid before archive");
  }
  if (rules.requireGeideaBeforeArchive && !order.geideaRegisteredAt) {
    return workflowError("Order must be registered on system first");
  }
